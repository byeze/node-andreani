...
if (existsSync(join(process.cwd(), "src", "credentials.json")))
  ...
  console.error(
    `Para ejecutar el test debe definir credenciales en ${process.cwd()}/src/credentials.json`
  );
  process.exit();
}
...
describe("Cotizaciones", () => {
  it("Pide cotización de un envío a domicilio", async function () {
    const data = await andreani.cotizarEnvioDomicilio({
      cpDestino: 2700,
      sucursalOrigen: "TOM",
      bultos: [
        {
  ...
  it("Pide cotización de un envío a sucursal", async function () {
    const data = await andreani.cotizarEnvioSucursal({
      cpDestino: 2700,
      sucursalOrigen: "TOM",
      bultos: [
        {
...
describe("Listados", () => {
  it("Obtiene listado de provincias", async function () {
    const data = await andreani.listarProvincias();
  ...
  it("Obtiene listado de sucursales", async function () {
    const data = await andreani.listarSucursales();
    expect(data).to.be.an("array");
...
describe("Ordenes", () => {
  it("Crear una nueva orden a domicilio", async function () {
    const data = await andreani.crearOrdenDomicilio({
      //contrato: "400006711",
      origen: {
        postal: {
          codigoPostal: "3378",
  ...
  it("Crear una nueva orden a sucursal", async function () {
    const data = await andreani.crearOrdenSucursal({
      //contrato: "400006711",
      origen: {
        postal: {
          codigoPostal: "3378",
  ...
  it("Obtiene una orden creada", async function () {
    const data = await andreani.obtenerOrden(NUMERO_ANDREANI);
