...
let credentials = require('../src/credentials.json')

const andreani = new Andreani( credentials )

andreani.crearOrdenSucursal({
    //contrato: "400006711",
    origen: {
        postal: {
            codigoPostal: "3378",
...
}).then( (x: any) => {console.log(JSON.stringify(x, null, 4))} ).catch( x => {console.log(x.response.data); console.log(x.config.headers)} ) 